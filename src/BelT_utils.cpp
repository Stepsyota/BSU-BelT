...
uint32_t BelT::ShLoNEW(uint32_t word, uint8_t r = 1) {
    return word >> r;
}
uint32_t BelT::ShHiNEW(uint32_t word, uint8_t r = 1) {
    return word << r;
}
uint32_t BelT::RotHiNEW(uint32_t word, uint8_t r = 32) {
    return ShHiNEW(word) ^ ShLoNEW(word, r);
}
...
uint32_t BelT::G_funcNEW(uint32_t word, uint8_t r) {
    ...
    uint32_t word_after_H = Connect8to32NEW(blocks);
    uint32_t result_word = RotHiNEW(word_after_H, r);
